...
6. 所有机台及所有磨前形状的砂轮目标吃入量统一为 `preform_thickness_mid * 0.6`；若自然开口大于 `product_length - 0.2`，必须移动砂轮圆心限制开口，机台配置不得覆盖该公共规则。
...
10. 下砂轮 R80 侧面缺口必须按三头机单导轨（下上）规则控制，料腔内部缺口开口必须小于产品长度，推荐限制为 `product_length - 0.2`。
...
8. 下砂轮料腔内部缺口仍必须满足 `lower_cavity_notch_opening <= product_length - 0.2`。
