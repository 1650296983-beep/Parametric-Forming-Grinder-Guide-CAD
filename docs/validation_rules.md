...
所有机台及所有磨前形状的砂轮目标吃入量均为 `preform_thickness_mid * 0.6`。若由此得到的 R80 自然开口大于 `product_length - 0.2`，校验必须确认生成器通过移动圆心限制开口，并使报告值、圆心、圆弧端点和相关 DIMENSION 定义点一致；机台配置不得用固定吃入量覆盖该公共规则。
