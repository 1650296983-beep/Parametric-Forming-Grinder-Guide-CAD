...
上述两条线只适用于磨前方块。模板若来自瓦型历史产品，原有四条线不得直接套用；磨前面包型应由平面和弧面重建三条，磨前瓦型应由上下弧面重建四条。弧端与弧顶是不同几何角色，不属于重复线。
...
- 两个砂轮同在上侧只描述机台结构；厚度间隙不再由机台推断，必须由任务中的“磨单边/高要求”显式选项决定
...
- 全部砂轮吃入使用公共规则 `preform_thickness_mid * 0.6`，机台模板不得以固定吃入量覆盖；R80 圆弧、型腔投影线断口和 R80 半径标注必须同步更新
...
双头机（上上）的砂轮排列不再自动触发 `0.09 mm` 厚度间隙；只有任务显式勾选“磨单边/高要求”才使用该值。侧面型腔投影线必须继承绿色 `3` 和 `DASHED`，不得保留模板图元的显式青色。
...
- `triple_double_down_up_up` 的截面中心上口固定为 `2.00 mm`，与其干净模板一致。
...
- `triple_double_up_up_up` 的截面中心上口固定为 `1.00 mm`，保持其干净模板原值，不复用下上上的 `2.00 mm`。
...
- 两节截面均必须保留并绑定 `section_center_opening` 尺寸；四处避空标注必须同时校验 DIMENSION 定义点和已渲染尺寸块的箭头/引线目标点。
