...
“磨单边/高要求”是任务输入中的显式工艺选项，不再由机台砂轮排列自动推断。未勾选时，磨前方块厚度间隙为 `0.12 mm`；勾选后所有机台统一使用 `0.09 mm`。机台配置不得保存或覆盖该间隙。

槽宽特殊间隙同样使用显式互斥选项：高对称度为 `0.03 mm`，大瓦放宽为 `0.08 mm`；未勾选时使用常规槽宽规则。

所有截面型腔采用六圆弧拓扑：四处侧部避空半径由用户输入的 `relief` 决定，中心两处始终固定为 `2-R0.50`。中心开口仍由机台模板配置，不得全局固定。
...
- `block_to_tile` 中，主 R 面位于第一砂轮侧；中心两处 R0.5 按机台模板的
  固定上口和平面拓扑生成。
- `block_to_tile` 的 `4-1 + 2-R0.50` 必须由同一个方向感知的瓦型构造器
  生成，适用于所有单导轨和双导轨机台；不得因为主 R 位于上侧就改走
  `block_to_bread`（面包型）构造分支。上 R 时，中心两处 R0.50 与主 R 的
  实际切点连接；下 R 时，中心两处 R0.50 与对侧平面连接。两种方向均必须
  保持四处侧部避空、两处中心避空、主 R 朝向和相邻端点连续可审计。
- `block_to_bread_rectangular` 中不生成成品主 R，中心两处固定 R0.5 与矩形
  槽上口连接。
- 用户把四处避空改为 `4-0.6` 时，只改变 `4-*` 四处，中心 `2-0.5` 仍保持 R0.5。
- 四处 `4-*` 必须保留两切点之间的外侧补弧：DXF 圆弧扫角必须大于
  `180°`，且弧中点位于对应槽边外侧（左侧中点 X 小于左槽边、右侧中点 X
  大于右槽边）。不得保留会穿入型腔的短内弧；该规则适用于全部机台、上下
  R 方向及双导轨的每一节。
